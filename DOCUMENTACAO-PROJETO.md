...
| 13/09/2026 | 5.3 | `firebase-blueprint.json`, `firestore.rules`, `src/services/firebase.ts`, `src/types.ts`, `server.ts`, `src/context/StoreContext.tsx`, `src/components/admin/AdminDashboard.tsx`, `src/components/master/MasterDashboard.tsx` | Implementação de persistência definitiva de senhas e usuários no banco de dados Firestore (`usuarios_staff`). Adicionado formulário de alteração de senha de admin nas Configurações da Loja e modal de alteração de senha de usuários no painel Master Admin. Sincronização automática entre Firestore e servidor. |
| 13/09/2026 | 5.4 | `src/context/StoreContext.tsx`, `vercel.json` | Correção crítica de autenticação em deploys estáticos na Vercel (`gamas-burger.vercel.app`). Adicionado fallback híbrido inteligente em `adminLogin` e verificação de inicialização: quando a API Node Express não está acessível no mesmo host, o sistema autentica diretamente pelo banco de dados Firestore (`usuarios_staff`) e credenciais de sistema, eliminando o erro "Erro de conexão com o servidor ao autenticar". Criado `vercel.json` para garantir roteamento SPA correto. |
